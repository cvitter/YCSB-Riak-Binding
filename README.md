...
4. Perform the following operations on your Riak cluster to configure Riak for the benchmarks:
  ...
  <ol>Upload the Solr search schema used to support YCSB's scan operation (<b>Note</b>: update the URL and file path to match your environment.)
```
...
```
  <ol>Create the "ycsb" bucket type and assign the ycsb search index to the bucket type.
```
